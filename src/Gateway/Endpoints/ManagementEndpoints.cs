    ...
    public static void MapManagementApi(this IEndpointRouteBuilder app)
    {
        ...
        api.MapGet("/operations/overview", async (ProxyOperationsQueryService queries, CancellationToken ct = default) =>
        {
            ...
            return Results.Ok(new
            {
                ...
                groups = overview.Groups.Select(item => new { groupId = item.GroupId, groupName = item.GroupName, requestsPerMinute = item.Attempts, observedEndpoints = item.ObservedEndpoints, measuredSuccessPercent = item.Attempts == 0 ? (double?)null : item.SuccessfulResponses * 100d / item.Attempts,
