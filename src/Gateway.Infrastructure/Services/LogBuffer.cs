...
public class LogBuffer : ILogBuffer
{
    ...
    public void AddInfo(string message, string? path = null, int? statusCode = null, double? durationMs = null) =>
        Add(new(DateTime.UtcNow, "Info", message, path, statusCode, durationMs));

    public void AddError(string message, string? path = null) =>
        Add(new(DateTime.UtcNow, "Error", message, path, null, null));

    public void AddWarning(string message, string? path = null) =>
        Add(new(DateTime.UtcNow, "Warning", message, path, null, null));
}
